...
- `setuptools>=61` already available to the same Python interpreter;
- Git for the recovery workflow;
- an official, reviewed ACGM Recover source tree whose `PACKAGE_MANIFEST.json` matches.

The installer deliberately uses `--no-index`, `--no-deps`, and `--no-build-isolation`. It will fail with a clear prerequisite report instead of downloading a missing build tool.

安装器固定使用 `--no-index`、`--no-deps` 和 `--no-build-isolation`。缺少构建工具时会明确失败，不会静默联网下载。
...
`bootstrap.py` first checks Python, pip, setuptools, and every source hash listed in `PACKAGE_MANIFEST.json`. Outside a virtual environment it performs this user installation without shell interpolation:

```text
PYTHON -m pip install --user --no-deps --no-build-isolation --no-index .
```
