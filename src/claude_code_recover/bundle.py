...
def _report_markdown(analysis: dict[str, Any]) -> str:
    ...
    return f"""# Claude Code Recover — Recovery Report / 恢复报告
...
Missing history remains a gap. Claude Code Recover does not fill missing sessions with a smoother story.

缺失历史会继续保留为缺口；Claude Code Recover 不会为了让故事连贯而补写不存在的证据。
...
You are reviewing a surviving software project from a Claude Code Recover bundle. Its recovery readiness is `{readiness}`.
...
def build_bundle(
    ...
    *,
    ...
    lock_path = final_output.parent / f".{final_output.name}.claude-code-recover.lock"
    ...
    try:
        ...
        staging_path = Path(
            tempfile.mkdtemp(prefix=f".{final_output.name}.claude-code-recover-staging-", dir=final_output.parent)
        )
