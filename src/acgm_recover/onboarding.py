"""Consent-preserving environment guidance for ACGM Recover."""
...
def environment_guide(
    ...
    *,
    ...
    next_commands = [
        ["PYTHON", "-m", "acgm_recover", "doctor", "--no-default-sources"],
    ]
    future_commands: list[list[str]] = []
    if route and runtime_supported:
        next_commands.append(["PYTHON", "-m", "acgm_recover", "discover", "SOURCE_OPTIONS"])
        future_commands.extend(
            [
                [
                    "PYTHON", "-m", "acgm_recover", "inspect", "--project", "PROJECT",
                    "SOURCE_OPTIONS",
                ],
                [
                    "PYTHON", "-m", "acgm_recover", "build", "--project", "PROJECT",
                    "--output", "NEW_BUNDLE", "--route", route, "SOURCE_OPTIONS",
                ],
            ]
        )
    elif route is None:
        next_commands.append(["PYTHON", "-m", "acgm_recover", "guide", "--route", "ROUTE"])
